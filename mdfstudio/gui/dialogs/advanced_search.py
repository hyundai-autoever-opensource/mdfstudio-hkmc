...
""" Edit history
    ...
    Functions
    ---------
    *   _add_window - Show add window button, Hide add button
    *   itemDoubleClicked - Add channels to selection box when double clicked
...
class AdvancedSearch(Ui_SearchDialog, QtWidgets.QDialog):
    def __init__(
        self, channels_db, return_names=False, show_add_window=True, *args, **kwargs
    ):
        ...
        self.matches.itemDoubleClicked.connect(self._add)
        self.selection.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        self.selection.customContextMenuRequested.connect(self.open_menu)
    ...
    def open_menu(self, position):
        ...
        action = menu.exec_(self.selection.viewport().mapToGlobal(position))

        if action is None:
            return

        if action.text() == "Delete (Del)":
            selected_items = self.selection.selectedItems()
            deleted = []
            for item in selected_items:
                row = self.selection.row(item)
                deleted.append(row)
                self.selection.takeItem(row)
