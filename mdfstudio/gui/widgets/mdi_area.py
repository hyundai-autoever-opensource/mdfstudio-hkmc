...
""" Edit history
    ...
    Functions
    ---------
    *   MdiAreaWidget.add_new_channels : Delete signals and build plot window again
        ...
        try:
            ...
            if with_tabular:
                if isinstance(widget.parent().parent().parent().parent(), Plot):
                    tabular_widget = widget
                if isinstance(widget, Plot):
                    tabular_widget = widget.tabular
                if isinstance(widget, Tabular):
                    tabular_widget = widget

                mdf_ = tabular_widget.mdf_
                new = []
                for sig in signals:
                ...
                channels = mdf_.iter_channels(copy_master=True)
                signals = []
                for sig in channels:
                    signals.append(sig)
            else:
            ...
            if isinstance(widget, Plot):
                if with_tabular:
                    sig_uuids = []
                    for row in range(widget.channel_selection.count()):
                        item = widget.channel_selection.item(row)
                        uuid = widget.channel_selection.itemWidget(item).uuid
                        sig_uuids.append(uuid)
                    widget.plot.delete_channels(sig_uuids)
                    widget.channel_selection.clear()
                    widget.info_uuid = None
