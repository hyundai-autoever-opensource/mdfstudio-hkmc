...
class FileWidget(WithMDIArea, Ui_file_widget, QtWidgets.QWidget):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for channel in self.channels_db_items:
            ...
            raster = float(self.mdf.get_channel_metadata(channel, self.mdf.channels_db[channel][0][0],self.mdf.channels_db[channel][0][1]).sampling_rate)
        ...
        for item in self.mdf.channels_db.items():
            for index in item[1]:
                gp_nr, ch_nr = index[0], index[1]
                grp = self.mdf.groups[gp_nr]
                channel = grp.channels[ch_nr]
                unit_group.add(self.mdf.get_channel_unit(channel.name,gp_nr,ch_nr))
        #
        # for group in self.mdf.groups:
            ...
            for i, unit in enumerate(unit_group):
                ...
                for i, group in enumerate(self.mdf.groups):
                    for j, ch in enumerate(group.channels):
                        ...
                        if unit == self.mdf.get_channel_unit(ch.name, i, j):
        ...
        if result:
            ...
            for signal in names:
                if signal[0] == "$":
                    ...
                    raster = float(self.mdf.get_channel_metadata(signal,self.mdf.channels_db[signal][0][0],self.mdf.channels_db[signal][0][1]).sampling_rate)
                    rasters.add(raster)
            ...
            if dlg.add_window_request:
                ...
                if not ok:
                    ...
                    if index == 0:
                        self.add_window(["Plot", signals, ret1])
                    elif index == 1:
                        self.add_window(["Tabular", signals, ret1])
                    elif index == 2:
                        self.add_window(["Plot+Tabular", signals, ret1])
                    else:
                        widgets = [
    ...
    def resample(self, mdf):
        ...
        mdf = mdf.resample(raster=raster, version=version, time_from_zero=time_from_zero)
    ...
    def cut(self, mdf, min, max):
        ...
        mdf = mdf.cut(start=start, stop=stop, whence=whence, version=version, time_from_zero=time_from_zero)
    ...
    def export(self, event):
        ...
        progress = setup_progress(
            parent=self,
            title="Export",
            message=f'Exporting "{self.file_name}"',
            icon_name="export4",
        )
        ...
        # new_mdf = self.mdf.filter(kwargs=kwargs)
        ...
        if file_name:
            ...
            if export_type in ("mdf", "mf4"):
                ...
                try:
                    ...
                    if '_ArrayMemoryError' in e.args[0]:
                        QtWidgets.QMessageBox.information(None, "Message",
                                                          "Out of Memory.\nUncheck Single Time base and try again.")
