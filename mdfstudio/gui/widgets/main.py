...
""" Edit history
    ...
    -   Insert About menu and Manual menu to menubar
    -   Add target information on window title
    -   Add main image on stackedwidget
...
class MainWindow(WithMDIArea, Ui_PyMDFMainWindow, QtWidgets.QMainWindow):
    ...
    def __init__(self, files=None, *args, **kwargs):
        ...
        open_group = QtWidgets.QActionGroup(self)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/images/manual1.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        action = QtWidgets.QAction(icon, "&User manual", menu)
        action.triggered.connect(self.help)
        action.setShortcut(QtGui.QKeySequence("F1"))
        open_group.addAction(action)
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(":/images/feedback.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        action = QtWidgets.QAction(icon, "&Send feedback", menu)
        ...
        pixmap = QtGui.QPixmap(':/images/main1.PNG')
    ...
    def webpage(self):
        webbrowser.open_new(r"http://e-tap.com:9300/21-00001-0-1.0")
